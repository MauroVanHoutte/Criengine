...
void TextRenderer::AddFont(const std::string& fontName, unsigned int size, const SDL_Color& color)
{
	...
	dae::Font font{ "../Data/" + fontName, size };
	...
	for (char i = 0; i < 127; i++)
	{
		...
		m_Fonts[fontId][i] = new dae::Texture2D(SDL_CreateTextureFromSurface(dae::Renderer::GetInstance().GetSDLRenderer(), surf));
...
void TextRenderer::RenderString(const std::string& text, const glm::vec3& location, const std::string& fontName, unsigned int size, const SDL_Color& color) const
{
	...
	for (const char& c : text)
	{
		...
		dae::Renderer::GetInstance().RenderTexture(*m_Fonts.find(fontId)->second.find(c)->second, location.x + xOffset, location.y + yOffset);
		xOffset += letterWidth;
	}
}
